...
class Parser implements TagConstants {
    ...
    private void parseBody(Node parent, String tag, String bodyType)
        ...
        if (bodyType.equalsIgnoreCase(TagInfo.BODY_CONTENT_TAG_DEPENDENT)) {
        ...
        } else if (bodyType.equalsIgnoreCase(JAVAX_BODY_CONTENT_PLUGIN)) {
            ...
            if (!reader.matchesETag(tag)) {
                ...
                || (bodyType.equalsIgnoreCase(JAVAX_BODY_CONTENT_PARAM))
                || (bodyType.equalsIgnoreCase(JAVAX_BODY_CONTENT_TEMPLATE_TEXT))) {
            while (reader.hasMoreInput()) {
                if (reader.matchesETag(tag)) {
                    return;
                }
                ...
                if (bodyType.equalsIgnoreCase(TagInfo.BODY_CONTENT_JSP)) {
                    ...
                    parseElementsScriptless(parent);
                } else if (bodyType.equalsIgnoreCase(JAVAX_BODY_CONTENT_PARAM)) {
                    // (note the == since we won't recognize JAVAX_*
                    // from outside this module).
                    reader.skipSpaces();
                    parseParam(parent);
                } else if (bodyType.equalsIgnoreCase(JAVAX_BODY_CONTENT_TEMPLATE_TEXT)) {
